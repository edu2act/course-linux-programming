...
函数递归调用实际上是系统在内存的栈空间不断调用函数，所以使用栈结构保存相应的数据就可以不使用递归。当然消除递归并非只能用栈结构，我们在后面的开发过程中会进行实际的操作。



到目前为止，这部分内容足够支撑后续的课程，以下内容是一些扩展内容。你不必要马上理解，如果你能很快理解固然很好，如果你目前理解起来有些困难也不必失去信心，随着开发能力提高，当你需要时可以回过头来再看以下内容。
