...
5. 有些人把信号描述为软件中断，这并不算是错。尽管有些信号是硬件产生或者是依赖于硬件，但实际会是内核进行处理，对程序员来说，只需要利用统一的接口操作即可。
...
!考虑到课程的时间问题，可靠信号作为选修内容，如果你没有时间或精力去看，这里要说明的是，可靠信号的机制兼容旧的信号模式，实际上signal的实现已经是基于可靠信号的处理方式来实现的，对接口调用来说，signal依旧封装成原来的调用方式，这样可以兼容原来的代码。
