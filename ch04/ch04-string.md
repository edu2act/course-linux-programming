...
学过数据结构和算法的课程，应该对KMP算法有所了解，但是KMP只具有理论上的性能，实际测试并不是很理想，由于Donald Ervin Knuth是此算法发明者之一，所以名气很大。但是字符串匹配最快的算法是BM算法。BM无论是理论性能还是实际测试都是最好的。你在代码编辑器或是其他一些文本操作工具中进行字符串搜索时，那些工具使用的都是BM算法。
