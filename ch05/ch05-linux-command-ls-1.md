...
这个程序功能十分简单，而且这仅仅是一个测试程序，但是接下来我们可以实现更复杂的功能，不过在实现之前要设计一下程序结构。注意这里的设计并不遵循极其严格的设计流程，程序会不断演进，甚至进行重构，以此发现程序开发随着复杂度提高，规模变大需要考虑的问题，尽管我们最终实现的版本也不会超过1000行代码，但是仍然不失为一个很好的例子。
